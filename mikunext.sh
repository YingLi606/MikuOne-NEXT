...
validity_git() {
	source "${HOME}/MikuOne-NEXT/config/config.sh"
	if [ "${git}" = "" ]; then
		wheregit=$(whiptail --title "请选择默认安装源" --menu "以后的每次安装会优先考虑默认安装源" 15 60 4 "1" "Github" "3" "Github加速代理" "0" "退出" 3>&1 1>&2 2>&3)
		case "${wheregit}" in
		1)
...
validity_auto_upgrade() {
	...
	if [ "${auto_upgrade}" = "" ]; then
		wheregit=$(whiptail --title "选择默认安装源" --menu "是否自动更新软件包(建议开启)" 15 60 4 "1" "开启" "2" "关闭" "0" "退出" 3>&1 1>&2 2>&3)
		case "${wheregit}" in
		1)
...
detect_package_manager() {
	...
	# 仅输出提示信息，通过变量返回包管理器名称，避免stdout混入提示内容
	echo -e "\n${INFO}正在检测包管理器（Termux环境）..."
	sleep 0.6
	if command -v "$package_manager" >/dev/null 2>&1; then
		echo -e "${SUSSEC}检测到的包管理器：$package_manager"
		echo "$package_manager"  # 最后一行仅输出包管理器名称，确保变量正确捕获
	else
...
# 捕获函数返回的包管理器（仅取最后一行stdout，即纯"pkg"）
package_manager=$(detect_package_manager | tail -n 1)
